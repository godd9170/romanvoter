  ...
  return (
    <React.Fragment>
      <div className="px-4 py-5 sm:px-6">
        ...
        <p className="mt-1 max-w-2xl text-sm text-gray-500">This is how everyone has voted.</p>
      </div>
          ...
          {voters.map((voter, i) => {
            return (
              <div key={i} className="px-4 py-5 flex px-6 border-b border-gray-200 last:border-b-0">
                <div className="px-2 text-4xl">{emojiMap[i % 3]}</div>
                <dt className="flex-grow text-lg font-medium text-gray-900 self-center">Full name</dt>
                <dd className="self-center h-6 w-6 text-green-600">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20" stroke="none">
                    <path fillRule="evenodd" clipRule="evenodd" d={i % 2 === 0 ? thumbsDownIcon : thumbsUpIcon} />
                  </svg>
                </dd>
              </div>
            )
